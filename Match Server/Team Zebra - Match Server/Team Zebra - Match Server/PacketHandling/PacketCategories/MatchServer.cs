...
{
    /// <summary>
    /// Contains the functions for all the MatchServer.___
    /// packets.
    /// </summary>
    public static class MatchServer
    {
        /// <summary>
        /// Sends the recommended channel to the
        /// given client.
        /// </summary>
        /// <param name="c">the client to send the response to</param>
        public static void responseRecommendedChannel(Client c)
        {
            // just send everyone to the development channel on login for now.
            GunzPacket response = new GunzPacket(0x64, 0x4B2, 19);
            response.startHeader();

            response.writeInt(0x00);
            response.writeInt(0x05);
